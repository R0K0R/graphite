...
export function initRoom(rootPath, customCode) {
  ...
  // Log every Y.Doc update: origin tells us local vs remote transport
  _doc.on('update', (update, origin) => {
    const src = origin === null ? 'local'
      : origin?.constructor?.name ?? String(origin);
    console.log(`[CRDT] Y.Doc update from="${src}" bytes=${update.byteLength}`);
  });

  webrtc.on('peers', ({ added, removed }) => {
    const conns = webrtc.room?.webrtcConns;
    const bcConns = webrtc.room?.bcConns;
    console.log('[CRDT] peers — added:', added, 'removed:', removed,
      '| webrtcConns:', conns?.size ?? '?',
      '| bcConns:', bcConns?.size ?? '?');
    if (conns) conns.forEach((conn, id) => {
      console.log('  webrtcConn', id, 'connected:', conn.connected, 'synced:', conn.synced);
    });
  });
  webrtc.on('synced', ({ synced }) => {
    console.log('[CRDT] synced:', synced);
  });
  _awareness.on('change', ({ added, updated, removed }) => {
    const t = Date.now();
    const states = _awareness.getStates();
    const changed = [...added, ...updated].map(id => {
      const s = states.get(id);
      return { id, name: s?.name, cursor: s?.cursor ? 'has-cursor' : 'no-cursor', focusedNode: s?.focusedNode };
    });
    console.log(`[CRDT] awareness @${t} added=${added} updated=${updated} removed=${removed}`, changed);
  });
